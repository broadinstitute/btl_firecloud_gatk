...
task gatk_alignbam_task {
    ...
    String aligned_bam_fn = "${sample_name}.aligned.sam"
    String sorted_bam_fn = "${sample_name}.sorted.sam"
    String marked_bam_fn = "${sample_name}.marked_duplicates.sam"
...
run('date')
run('java -Xms400G -jar ${picard_path} SamToFastq  INPUT=${in_bam} FASTQ=${fq1_fn} SECOND_END_FASTQ=${fq2_fn} VALIDATION_STRINGENCY=LENIENT')


run('echo STARTING bwa mem')
run('date')
run('bwa mem -t 32 -R ${read_group} ref.fasta ${fq1_fn} ${fq2_fn}  > ${aligned_bam_fn}')

run('echo STARTING SortSam')
run('date')
run('java -Xms400G -jar ${picard_path} SortSam  I=${aligned_bam_fn} O=${sorted_bam_fn} SO=coordinate')

run('echo STARTING MarkDuplicates')
run('date')
run('java -Xms400G -jar ${picard_path} MarkDuplicates I=${sorted_bam_fn} O=${marked_bam_fn} M=${marked_duplicates_metrics_fn}')

run('echo STARTING ReorderSam')
run('date')
run('java -Xms400G -jar ${picard_path} ReorderSam   I=${marked_bam_fn} O=${out_bam_fn}.sam R=ref.fasta')

run('echo STARTING sam to bam')
run('date')
run('samtools view -bS ${out_bam_fn}.sam > ${out_bam_fn}')
